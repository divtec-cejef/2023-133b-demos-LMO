...
if(isset($_POST['v_frm_identite']) === true)
{
    ...
    if ((empty($nom) === false) && (empty($mdp) === false)) {
        ...
        foreach ($authentificationsValides as $unAuth) {
            $identifiantsValides[] = $unAuth['name'];
        }

        if (in_array($nom,$identifiantsValides) === true) {
            //if ($mdp === $authentificationsValides[key($identifiantsValides,$nom)]['mdp']) {
            $key = array_search($nom,$identifiantsValides);
            $mdp_hash = $authentificationsValides[$key]['mdp'];
            $user_niveau = $authentificationsValides[$key]['niveau'];

            if (password_verify($mdp, $mdp_hash) === true) {
